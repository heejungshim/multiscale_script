...
##
## Example Usage (see command in /home/hjshim/d/hjshim/projects/multiscale/atacseq_analysis/run/multiscale/com/Copper.1024.both.alt.post) : R CMD BATCH --no-save --no-restore "--args chr=1 st.sites=1 en.sites=28978 path.output.dir='/home/hjshim/d/hjshim/projects/multiscale/atacseq_analysis/run/multiscale/Copper.1024.both.alt.output/' path.sum.dir='/home/hjshim/d/hjshim/projects/multiscale/atacseq_analysis/run/multiscale/Copper.1024.both.alt.sum/' output.file.name='Copper.1024.both.alt'" /home/hjshim/d/hjshim/projects/multiscale/multiscale_script/R/get.logLR.ms.R
##
##
...
##st.sites=1
##en.sites=28978
##path.output.dir='/home/hjshim/d/hjshim/projects/multiscale/atacseq_analysis/run/multiscale/Copper.1024.both.alt.output/'
##path.sum.dir='/home/hjshim/d/hjshim/projects/multiscale/atacseq_analysis/run/multiscale/Copper.1024.both.alt.sum/'
...
for(i in st.sites:en.sites){
    ...
    if(file.exists(path.each)== FALSE){		
        ...
        if(file.info(path.each)$size == 0){
          ...
          dat = scan(path.each, what=double())
          done[i] = TRUE
          if(!is.na(dat[1])){
            logLR[i] = as.numeric(dat[1])
          }
        }
      }
  }
